...
from predictor import score_ticker, rank_top_picks
from screener import MAX_PRICE

# explainer.py wasn't part of the files shared for this rewrite - if you add
# it back in, this import gives you a proper per-pick explanation; otherwise
# the fallback below produces a plain-text reason so app.py still runs.
try:
    from explainer import build_explanation
except Exception:
    def build_explanation(features, prediction):
        return (
            f"{features['ticker']}: momentum {features['momentum']:+.1f}%, "
            f"relative volume {features['relative_volume']:.2f}x, "
            f"news sentiment {features['news_sentiment']:.2f} -> "
            f"predicted {prediction['predicted_direction']} "
            f"({prediction['expected_move_pct']:.1f}% expected, "

...
trusting this for money.

NO FALLBACK: refresh_data() does not catch failures and serve stale/sample
data - if the live pull fails, it raises and the API returns a clean error.
The one exception is inside try_live_pull()'s per-ticker loop: a single
ticker's fetch error is skipped (that ticker just isn't included) rather than
aborting the whole batch. That's a deliberate choice, not a silent
degradation of the overall response - flag it if you'd rather the whole
refresh fail when any one candidate errors.
"""
...
def refresh_data(max_price: float = MAX_PRICE) -> Dict[str, Any]:
    """Pulls live data. No fallback: if the live pull fails, this raises and
    the caller (app.py) turns that into a clean failure response - it does
    NOT silently serve sample_market_data.json instead."""
